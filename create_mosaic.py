...
def main():
	...
	output = output.resize((int(target_image_width*k/2), int(target_image_height*k/2)), Image.Resampling.LANCZOS)
...
def check_pixel_batch_size(pixel_batch_size, img, max_iterations=100):
    '''
    Function to adjust Pixel Batch Size so it is the
    multiples of image's height and width
    '''
    if max_iterations <= 0:
        raise ValueError("Max recursion limit reached, unable to find valid pixel_batch_size")

    if (img.shape[0] % pixel_batch_size == 0) and (img.shape[1] % pixel_batch_size == 0):
        print(f"Using Pixel Batch Size: {pixel_batch_size}")
        return pixel_batch_size
    else:
        # Increase pixel_batch_size in increments of 1 (or another value as necessary)
        pixel_batch_size += 1
        return check_pixel_batch_size(pixel_batch_size, img, max_iterations - 1)
